...
void __ISR (_ADC_VECTOR, ipl6) ADCHandler (void)
{
  ...
  ConfigIntADC10(ADC_INT_PRI_6 | ADC_INT_SUB_PRI_2 | ADC_INT_OFF);

  // Buffer offset to point to the base of the idle buffer
  u32 offset = 8 * ((~ReadActiveBufferADC10() & 0x01));
  idx8 ch;
  #ifdef _AD_FLT_
  s32 dd;
  s32 ii;
  ...
  for (ch = 0; ch < ad_num; ch++)
    {
      ...
      #ifdef _AD_FLT_
      dd = ((512U-128U+ad_adj[ch])*ReadADC10(offset + ch))/1024U; /* Adjust */
      ad_res[ch] += (dd - ad_res[ch])/ad_fk[ch];
      #else /*_AD_FLT_*/
      ad_res[ch] = ReadADC10(offset + ch);
      #endif /*_AD_FLT_*/
    }
