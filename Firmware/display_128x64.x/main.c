...
volatile u32 led_count;
volatile u32 USBActive;
bool isSSD1306 = false;
...
void __ISR(_TIMER_1_VECTOR, ipl2) Timer1Handler(void) //1ms
{
  ...
  if (USBActive) USBActive--;

  mPORTBToggleBits(BIT_4); // Pin 12
  mPORTBSetBits(BIT_5); // Pin 14

  // Clear the interrupt flag
  INTClearFlag(INT_T1);

  mAD1IntEnable(1);     //Enable ADC interrupt

}

static void wait_ms(int ms)
{
 int init_ms = __1ms_cycle;

 while((__1ms_cycle - init_ms) < ms) ClearWDT();
}
...
//---------------------------------------------------------------------------
void d2scistr(char *str, const char *value, double d, const char *measure)
{
 ...
 if (d < 0) dd = -dd;
 if (rng == 8) sprintf(str, "%s%.2f%s", value, d, measure);
 else
 if ((rng >= 0) && (rng < 17))
  sprintf(str, "%s%.2f%c%s", value, dd, csci[rng], measure);
 else
  sprintf(str, "%s%.2e %s", value, d, measure);
}
//---------------------------------------------------------------------------

void ShowLightLevel(u8 x, u8 y)
{
 char str[16];
 static double AvgLux=0;

 int oSt = GetOPTreg(0, 1); // Read Reg @1 (Configuration Register)
 if (oSt < 0) text("No light data.  ", x, y);
 else
 if ((oSt & 0xfe00) != 0xcc00) SetOPTreg(0, 1, 0xcc00);
 else
 if (oSt & (1<<7)) //Conversion ready flag
  {
   double lux = MasterMul*GetOPT(0);
   if (lux >= 0)
    {
     //Low pass filter
      ...
      if (sec != prev_sec)
       {
        ...
        setTextSize(2);
        if (!USBActive) ShowLightLevel(0, 0);
        else text("    USB     ", 0, 0);
        //sprintf(str, "%4d", sec%10000);
        //text(str, 72, 32);
        isSSD1306 = display();
       }
     }
...
void UserInit(void)
{
    ...
    MasterMul =  read_val(eeMasterMul);
    if (MasterMul < 0) MasterMul = 1;

    wait_ms(100);
    gfx_init();
    isSSD1306 = ssd1306_init(); //
    if (isSSD1306) isSSD1306 = display(); //Show logo
    if (isSSD1306)
     {
      char str[16];
      wait_ms(1000);
...
//      drawCircle(20, 20, 8, WHITE);
//      drawLine(5,50, 100, 5, WHITE);
//      fillRect(100, 50, 10, 10, WHITE);
//      drawChar(30, 50, 'Z',WHITE, WHITE, 2);
//
//      setCursor(64, 32);
//      setTextSize(1);
//      setTextBgColor(WHITE, BLACK);
...
//      if (isSSD1306) isSSD1306 = display();
//      wait_ms(2000);
//      if (isSSD1306) isSSD1306 = dim(true);
//      sprintf(str, "%1.3f", MasterMul);
//      text(str, 48, 40);
//     }
              ...
              if (!HIDTxHandleBusy(USBInHandle))
                {
                  ...
                  ToSendDataBuffer[5] = ad_res[2] % 256;
                  ToSendDataBuffer[6] = ad_res[2] / 256;
...
void BlinkUSBStatus(void)
{
 ...
 if (led_count == 0) led_count = LED_PERIOD_ms;//250;
}//end BlinkUSBStatus
