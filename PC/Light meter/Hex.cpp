...
CHexManager::~CHexManager()
{
  ...
  if(HexFilePtr) fclose(HexFilePtr);
}
 ...
 *****************************************************************************/
bool CHexManager::LoadHexFile(const AnsiString fName)
{
  char HexRec[255];

  HexFilePath = fName;
  // Open file
  HexFilePtr = fopen(HexFilePath.c_str(), "r");
  if(HexFilePtr == NULL)
   {
